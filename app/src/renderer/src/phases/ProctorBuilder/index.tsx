...
/**
 * Top-level proctor builder screen. Owns:
 *   - the people roster fetched once on mount
 *   - the local builder reducer state (selection + config knobs)
 *   - save / load / back actions
 *
 * Subcomponents are pure-ish — this file is the only place that talks
 * to `window.api` for the builder.
 */
export function ProctorBuilder() {
  ...
  function buildConfig(): StudyConfig | null {
    ...
    // Preserve the on-screen order (which mirrors the people roster from main).
